...
def main():
    ...
    for _ in range(num_passwords):
        ...
        if l33t_flag is True:
            print(l33t(text=new_password, mutations=mutations))
        else:
            print(new_password)
...
def l33t(text: str, mutations: float) -> str:
    text = ransom(text)
    xform = str.maketrans({
    ...
    for i in random.sample(range(len_text), mutation_num):
        new_char = new_text[i].translate(xform)
        new_text = new_text[:i] + new_char + new_text[i + 1:]
    return new_text + random.choice(string.punctuation)
...
def test_l33t():
    state = random.getstate()
    random.seed(1)
    assert l33t('Money', 0.1) == 'moNeY{'
    assert l33t('Dollars', 0.1) == 'DOllArS!'
    random.setstate(state)
